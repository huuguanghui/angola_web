...
<%
	Integer errorCode = (Integer) request
			.getAttribute(UserController.ErrorCode);
	...
	Integer phoneNumberError = (Integer) request
			.getAttribute(UserController.PhoneNumberError);
	...
	Integer phoneCodeError = (Integer) request
			.getAttribute(UserController.PhoneCodeError);
	...
	Integer passwordError = (Integer) request
			.getAttribute(UserController.PasswordError);
	...
	Integer confirmPasswordError = (Integer) request
			.getAttribute(UserController.ConfirmPasswordError);
		...
		<div class="row">
			...
			<form id="formSignup" action="/uutalk/user/websignup" method="post"
				...
				<div id="divPhoneNumberCtrl"
					class="control-group <%=phoneNumberErrorInfo.isEmpty() ? "" : "error"%>">
					...
					<div class="controls">
						...
						<span id="spanPhoneNumberInfo" class="help-inline"> <%=phoneNumberErrorInfo%>
						</span>
					</div>
				</div>
				<div
					class="control-group <%=phoneCodeErrorInfo.isEmpty() ? "" : "error"%>">
					<label class="control-label" for="iptPhoneCode">验证码</label>
					<div class="controls">
						<input type="text" name="phoneCode" id="iptPhoneCode"
							pattern="[0-9]{6}" maxlength="6" value="<%=phoneCode%>" /> <span
							id="spanPhoneCodeInfo" class="help-inline"> <%=phoneCodeErrorInfo%>
						</span>
					</div>
				</div>
				<div
					class="control-group <%=passwordErrorInfo.isEmpty() ? "" : "error"%>">
					<label class="control-label" for="iptPassword">密码</label>
					<div class="controls">
						<input type="password" name="password" id="iptPassword" /> <span
							id="spanPasswordInfo" class="help-inline"> <%=passwordErrorInfo%>
						</span>
					</div>
				</div>
				<div
					class="control-group <%=confirmPasswordErrorInfo.isEmpty() ? "" : "error"%>">
					<label class="control-label" for="iptConfirmPassword">确认密码</label>
					<div class="controls">
						<input type="password" name="confirmPassword"
							id="iptConfirmPassword" /> <span id="spanConfirmPasswordInfo"
							class="help-inline"> <%=confirmPasswordErrorInfo%>
						</span>
					</div>
				</div>
				<hr>
